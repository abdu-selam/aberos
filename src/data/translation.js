export const header = {
  en: {
    name: "Aberos",
    navs: [
      {
        name: "Home",
        link: "home",
      },
      {
        name: "About",
        link: "about",
      },
      {
        name: "Companies",
        link: "companies",
      },
      {
        name: "Contact Us",
        link: "contact",
      },
    ],
  },
  ar: {
    name: "أبيروس",
    navs: [
      {
        name: "بيت",
        link: "home",
      },
      {
        name: "معلومات عنا",
        link: "about",
      },
      {
        name: "شركات",
        link: "companies",
      },
      {
        name: "اتصل بنا",
        link: "contact",
      },
    ],
  },
  am: {
    name: "አበሮስ",
    navs: [
      {
        name: "መግቢያ",
        link: "home",
      },
      {
        name: "ስለኛ",
        link: "about",
      },
      {
        name: "ድርጅቶች",
        link: "companies",
      },
      {
        name: "መልክት አሎት?",
        link: "contact",
      },
    ],
  },
};

export const home = {
  en: {
    hero: {
      title: 'Aberos',
      desc: "Your journey starts here.Discover what we do, how we do it, and how we can help you achieve more.",
      btns: ["Learn More", "Watch Video"],
    },
    video: {
      title: "Inside Our Company",
      desc: "Discover how Aberos PLC operates across multiple industries.",
      link: "https://www.youtube-nocookie.com/embed/bWzN8NuZWF8?controls=0&modestbranding=1",
    },
    about: {
      title: "About Us",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Incidunt, id dolores consequatur voluptatum facilis expedita.",
      btn: "Read More",
    },
    companies: {
      title: " Our Subsidiaries",
      desc: "Each company operates independently while sharing a unified vision of excellence.",
      each: [
        {
          title: "Manufacturing",
          desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Incidunt, id dolores consequatur voluptatum facilis expedita.",
          link: "manufacture",
          btn: "Explore",
        },
        {
          title: "Real Estate",
          desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Incidunt, id dolores consequatur voluptatum facilis expedita.",
          link: "restaurant",
          btn: "Explore",
        },
        {
          title: "Aberos Mall",
          desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Incidunt, id dolores consequatur voluptatum facilis expedita.",
          link: "mall",
          btn: "Explore",
        },
      ],
    },
    whyUs: {
      title: "Why Choose Us",
      items: [
        {
          title: "Lorem ipsum",
          desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ut ipsam facere quibusdam, quidem deleniti veritatis nobis nesciunt voluptatem vero in!",
        },
        {
          title: "Lorem ipsum",
          desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ut ipsam facere quibusdam, quidem deleniti veritatis nobis nesciunt voluptatem vero in!",
        },
        {
          title: "Lorem ipsum",
          desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ut ipsam facere quibusdam, quidem deleniti veritatis nobis nesciunt voluptatem vero in!",
        },
        {
          title: "Lorem ipsum",
          desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Ut ipsam facere quibusdam, quidem deleniti veritatis nobis nesciunt voluptatem vero in!",
        },
      ],
    },
  },
  ar: {
    hero: {
      desc: "لوريم إيبسوم، هو نص وهمي يُستخدم في صناعة الطباعة والتصميم. الأشياء التي تُعتبر متعة حقيقية هي صحراء، تُنفر، لا أعرف كما لو، المكاتب نفسها، هذا سبب أكبر هن",
      btns: ["ابحث عنا", "شاهد الفيديو"],
    },
    video: {
      title: "داخل شركتنا",
      desc: "اكتشف كيف تعمل شركة Aberos PLC في مختلف الصناعات.",
      link: "https://www.youtube-nocookie.com/embed/bWzN8NuZWF8?controls=0&modestbranding=1",
    },
    about: {
      title: "معلومات عنا",
      desc: "لوريم إيبسوم، هو نص وهمي يُستخدم في صناعة الطباعة والتصميم. الأشياء التي تُعتبر متعة حقيقية هي صحراء، تُنفر، لا أعرف كما لو، المكاتب نفسها، هذا سبب أكبر هنا يمكننا جميعًا التمييز، نفترض المخترع. على استعداد لشرح كل شيء،",
      btn: "شاهد المزيد",
    },
    companies: {
      title: "شركاتنا التابعة",
      desc: "تعمل كل شركة بشكل مستقل مع مشاركة رؤية موحدة للتميز.",
      each: [
        {
          title: "صناعة الطباعة ",
          desc: "لوريم إيبسوم، هو نص وهمي يُستخدم في صناعة الطباعة والتصميم. الأشياء التي تُعتبر متعة حقيقية هي صحراء، تُنفر، لا أعرف كما لو، المكاتب نفسها",
          link: "manufacture",
          btn: "أن تعرف جيداً",
        },
        {
          title: "صناعة الطباعة ",
          desc: "لوريم إيبسوم، هو نص وهمي يُستخدم في صناعة الطباعة والتصميم. الأشياء التي تُعتبر متعة حقيقية هي صحراء، تُنفر، لا أعرف كما لو، المكاتب نفسها،",
          link: "restaurant",
          btn: "أن تعرف جيداً",
        },
        {
          title: "صناعة الطباعة ",
          desc: "لوريم إيبسوم، هو نص وهمي يُستخدم في صناعة الطباعة والتصميم. الأشياء التي تُعتبر متعة حقيقية هي صحراء، تُنفر، لا أعرف كما لو، المكاتب نفسها،",
          link: "mall",
          btn: "أن تعرف جيداً",
        },
      ],
    },
    whyUs: {
      title: "Why Choose Us",
      items: [
        {
          title: "هو نص وهم",
          desc: "لوريم إيبسوم، هو نص وهمي يُستخدم في صناعة الطباعة والتصميم. الأشياء التي تُعتبر متعة حقيقية هي صحراء، تُنفر، لا أعرف كما لو، المكاتب نفسها، هذا سبب أكبر هنا يمكننا جم",
        },
        {
          title: "هو نص وهم",
          desc: "لوريم إيبسوم، هو نص وهمي يُستخدم في صناعة الطباعة والتصميم. الأشياء التي تُعتبر متعة حقيقية هي صحراء، تُنفر، لا أعرف كما لو، المكاتب نفسها، هذا سبب أكبر هنا يمكننا جم",
        },
        {
          title: "هو نص وهم",
          desc: "لوريم إيبسوم، هو نص وهمي يُستخدم في صناعة الطباعة والتصميم. الأشياء التي تُعتبر متعة حقيقية هي صحراء، تُنفر، لا أعرف كما لو، المكاتب نفسها، هذا سبب أكبر هنا يمكننا جم",
        },
        {
          title: "هو نص وهم",
          desc: "لوريم إيبسوم، هو نص وهمي يُستخدم في صناعة الطباعة والتصميم. الأشياء التي تُعتبر متعة حقيقية هي صحراء، تُنفر، لا أعرف كما لو، المكاتب نفسها، هذا سبب أكبر هنا يمكننا جم",
        },
      ],
    },
  },
  am: {
    hero: {
      title: 'አበሮስ',
      desc: "ጉዞዎ እዚህ ይጀምራል።ስለ እኛ ይወቁ፣እንዴት እንደምንሰራ ይረዱ እና የበለጠ አብረን እንደግ! ",
      btns: ["በደንብ ለማወቅ", "ቪዲዮ ይመልከቱ"],
    },
    video: {
      title: "በእኛ ኩባንያ ውስጥ",
      desc: "አበሮስ ኃ.የተ.የግ.ማ. በተለያዩ ኢንዱስትሪዎች ውስጥ እንዴት እንደሚሰራ ለማወቅ ቪዲዮውን ይመልክቱ",
      link: "https://www.youtube-nocookie.com/embed/bWzN8NuZWF8?controls=0&modestbranding=1",
    },
    about: {
      title: "ስለ እኛ",
      desc: "ሎሬም፣ ipsum dolor sit amet consectetur adipisicing elit። ነገሮች በእውነት ነፃ የሆኑ ደስታ በረሃዎች ናቸው፣ ይገፉ፣ እንደዛ እንደሆነ አላውቅም፣ ቢሮዎች እራሳቸው እነዚህ",
      btn: "የበለጠ ልማወቅ",
    },
    companies: {
      title: "የእኛ ድርጅቶች",
      desc: "እያንዳንዱ ኩባንያ ራሱን ችሎ የሚሰራ ሲሆን አንድ የሆነ ራዕይን ይዞ ይንቀሳቀሳል",
      each: [
        {
          title: "Manufacturing",
          desc: "ሎሬም፣ ipsum dolor sit amet consectetur adipisicing elit። ነገሮች በእውነት ነፃ የሆኑ ደስታ በረሃዎች ናቸው፣ ይገፉ፣ እንደዛ እንደሆነ አላውቅም፣ ቢሮዎች እራሳቸው እነዚህ ትልቅ ምክንያቶች እዚህ ሁላችን",
          link: "manufacture",
          btn: "በደንብ ለማወቅ",
        },
        {
          title: "Real Estate",
          desc: "ሎሬም፣ ipsum dolor sit amet consectetur adipisicing elit። ነገሮች በእውነት ነፃ የሆኑ ደስታ በረሃዎች ናቸው፣ ይገፉ፣ እንደዛ እንደሆነ አላውቅም፣ ቢሮዎች እራሳቸው እነዚህ ትልቅ ምክንያቶች እዚህ ሁላችን",
          link: "restaurant",
          btn: "በደንብ ለማወቅ",
        },
        {
          title: "Aberos Mall",
          desc: "ሎሬም፣ ipsum dolor sit amet consectetur adipisicing elit። ነገሮች በእውነት ነፃ የሆኑ ደስታ በረሃዎች ናቸው፣ ይገፉ፣ እንደዛ እንደሆነ አላውቅም፣ ቢሮዎች እራሳቸው እነዚህ ትልቅ ምክንያቶች እዚህ ሁላችን",
          link: "mall",
          btn: "በደንብ ለማወቅ",
        },
      ],
    },
    whyUs: {
      title: "Why Choose Us",
      items: [
        {
          title: "ሎሬም፣ ipsum",
          desc: "ሎሬም፣ ipsum dolor sit amet consectetur adipisicing elit። ነገሮች በእውነት ነፃ የሆኑ ደስታ በረሃዎች ናቸው፣ ይገፉ፣ እንደዛ እንደሆነ አላውቅም፣ ቢሮዎች እራሳቸው እነዚህ ትልቅ ምክንያቶች እዚህ ሁላችንም ልዩነት መፍጠር እንችላለን። ",
        },
        {
          title: "ሎሬም፣ ipsum",
          desc: "ሎሬም፣ ipsum dolor sit amet consectetur adipisicing elit። ነገሮች በእውነት ነፃ የሆኑ ደስታ በረሃዎች ናቸው፣ ይገፉ፣ እንደዛ እንደሆነ አላውቅም፣ ቢሮዎች እራሳቸው እነዚህ ትልቅ ምክንያቶች እዚህ ሁላችንም ልዩነት መፍጠር እንችላለን። ",
        },
        {
          title: "ሎሬም፣ ipsum",
          desc: "ሎሬም፣ ipsum dolor sit amet consectetur adipisicing elit። ነገሮች በእውነት ነፃ የሆኑ ደስታ በረሃዎች ናቸው፣ ይገፉ፣ እንደዛ እንደሆነ አላውቅም፣ ቢሮዎች እራሳቸው እነዚህ ትልቅ ምክንያቶች እዚህ ሁላችንም ልዩነት መፍጠር እንችላለን። ",
        },
        {
          title: "ሎሬም፣ ipsum",
          desc: "ሎሬም፣ ipsum dolor sit amet consectetur adipisicing elit። ነገሮች በእውነት ነፃ የሆኑ ደስታ በረሃዎች ናቸው፣ ይገፉ፣ እንደዛ እንደሆነ አላውቅም፣ ቢሮዎች እራሳቸው እነዚህ ትልቅ ምክንያቶች እዚህ ሁላችንም ልዩነት መፍጠር እንችላለን። ",
        },
      ],
    },
  },
};

export const contact = {
  en: {
    title: "Contact Us",
    desc: "You’re welcome to share anything you wish—your message will be read carefully.",
    name: "name",
    email: "email",
    comment: "write your message here",
    btn: "Send Message",
  },
  ar: {
    title: "اتصل بنا",
    desc: "يمكنك مشاركة أي شيء ترغب فيه، وسيتم قراءة رسالتك بعناية.",
    name: "اسم",
    email: "جيميل",
    comment: "اكتب رسائلك هنا",
    btn: "إرسال الرسائل",
  },
  am: {
    title: "መልእክቶን ያስቀምጡ",
    desc: "የሚፈልጉትን ማንኛውንም ነገር ይጻፉልን - እኛ መልእክትዎን በሚገባ እናነባለን!",
    name: "ስም",
    email: "ኢሜል",
    comment: "መልእክቶን እዚህ ጋር ያስቅምጡ",
    btn: "መልክቱ ይላክ",
  },
};

export const about = {
  en: {
    hero: {
      title: "About",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Adipisci,dolorum.",
    },
    history: {
      title: "Our History",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Sint eius accusantium iure harum odit. Quidem incidunt repudiandae saepe ipsa itaque soluta veniam quibusdam cum ex voluptas? Natus similique saepe assumenda architecto asperiores excepturi repudiandae eum repellat atque voluptatum aperiam praesentium inventore, veniam iure totam esse officiis commodi ipsum. Nobis, distinctio.",
    },
    mission: {
      title: "Our Mission",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Laborum praesentium fugiat corrupti! Perspiciatis quae dolore repudiandae enim sunt quam debitis vitae quia, nobis iure tempora.",
    },
    vision: {
      title: "Our Vision",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Laborum praesentium fugiat corrupti! Perspiciatis quae dolore repudiandae enim sunt quam debitis vitae quia, nobis iure tempora.",
    },
    business: {
      title: " Our Businesses",
      busines: [
        {
          name: "Manufacturing",
          desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Laborum praesentium fugiat corrupti! Perspiciatis quae dolore repudiandae enim sunt quam debitis vitae quia, nobis iure tempora.",
          link: "manufacture",
          btn: "See More",
        },
        {
          name: "Real Estate",
          desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Laborum praesentium fugiat corrupti! Perspiciatis quae dolore repudiandae enim sunt quam debitis vitae quia, nobis iure tempora.",
          link: "restaurant",
          btn: "See More",
        },
        {
          name: "Retail Mall",
          desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Laborum praesentium fugiat corrupti! Perspiciatis quae dolore repudiandae enim sunt quam debitis vitae quia, nobis iure tempora.",
          link: "mall",
          btn: "See More",
        },
      ],
    },
  },
  am: {
    hero: {
      title: "About",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Adipisci,dolorum.",
    },
    history: {
      title: "Our History",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Sint eius accusantium iure harum odit. Quidem incidunt repudiandae saepe ipsa itaque soluta veniam quibusdam cum ex voluptas? Natus similique saepe assumenda architecto asperiores excepturi repudiandae eum repellat atque voluptatum aperiam praesentium inventore, veniam iure totam esse officiis commodi ipsum. Nobis, distinctio.",
    },
    mission: {
      title: "Our Mission",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Laborum praesentium fugiat corrupti! Perspiciatis quae dolore repudiandae enim sunt quam debitis vitae quia, nobis iure tempora.",
    },
    vision: {
      title: "Vision",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Laborum praesentium fugiat corrupti! Perspiciatis quae dolore repudiandae enim sunt quam debitis vitae quia, nobis iure tempora.",
    },
    business: {
      title: " Our Businesses",
      busines: [
        {
          name: "Manufacturing",
          desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Laborum praesentium fugiat corrupti! Perspiciatis quae dolore repudiandae enim sunt quam debitis vitae quia, nobis iure tempora.",
          link: "manufacture",
          btn: "See More",
        },
        {
          name: "Real Estate",
          desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Laborum praesentium fugiat corrupti! Perspiciatis quae dolore repudiandae enim sunt quam debitis vitae quia, nobis iure tempora.",
          link: "restaurant",
          btn: "See More",
        },
        {
          name: "Retail Mall",
          desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Laborum praesentium fugiat corrupti! Perspiciatis quae dolore repudiandae enim sunt quam debitis vitae quia, nobis iure tempora.",
          link: "mall",
          btn: "See More",
        },
      ],
    },
  },
  ar: {
    hero: {
      title: "About",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Adipisci,dolorum.",
    },
    history: {
      title: "Our History",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Sint eius accusantium iure harum odit. Quidem incidunt repudiandae saepe ipsa itaque soluta veniam quibusdam cum ex voluptas? Natus similique saepe assumenda architecto asperiores excepturi repudiandae eum repellat atque voluptatum aperiam praesentium inventore, veniam iure totam esse officiis commodi ipsum. Nobis, distinctio.",
    },
    mission: {
      title: "Our Mission",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Laborum praesentium fugiat corrupti! Perspiciatis quae dolore repudiandae enim sunt quam debitis vitae quia, nobis iure tempora.",
    },
    vision: {
      title: "Vision",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Laborum praesentium fugiat corrupti! Perspiciatis quae dolore repudiandae enim sunt quam debitis vitae quia, nobis iure tempora.",
    },
    business: {
      title: " Our Businesses",
      busines: [
        {
          name: "Manufacturing",
          desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Laborum praesentium fugiat corrupti! Perspiciatis quae dolore repudiandae enim sunt quam debitis vitae quia, nobis iure tempora.",
          link: "manufacture",
          btn: "See More",
        },
        {
          name: "Real Estate",
          desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Laborum praesentium fugiat corrupti! Perspiciatis quae dolore repudiandae enim sunt quam debitis vitae quia, nobis iure tempora.",
          link: "restaurant",
          btn: "See More",
        },
        {
          name: "Retail Mall",
          desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Laborum praesentium fugiat corrupti! Perspiciatis quae dolore repudiandae enim sunt quam debitis vitae quia, nobis iure tempora.",
          link: "mall",
          btn: "See More",
        },
      ],
    },
  },
};

export const companies = {
  en: {
    hero: {
      title: "Our Companies",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Adipisci, dolorum.",
    },
    navs: [
      {
        name: "Mall",
        link: "mall",
      },
      {
        name: "Restaurant",
        link: "restaurant",
      },
      {
        name: "Manufacturing",
        link: "manufacture",
      },
    ],
    video: {
      title: "Inside Our Company",
      desc: "Discover how Aberos PLC operates across multiple industries.",
      link: "https://www.youtube.com/embed/bWzN8NuZWF8?controls=0&modestbranding=1",
    },
    comp: [
      {
        name: "Mall",
        key: "mall",
        desc1:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Adipisci, dolorum.",
        desc2:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur earum sapiente ratione excepturi impedit animi autem ea vero nisi aliquid maiores cum repellendus sed tenetur nostrum assumenda alias. Earum nihil voluptatibus aliquid rerum illo temporibus commodi eaque neque voluptate recusandae nam, totam cupiditate repudiandae blanditiis rem?",
        desc3:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur",
        video:
          "https://www.youtube-nocookie.com/embed/bWzN8NuZWF8?controls=0&modestbranding=1",
        map: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d988.1115485527807!2d38.18581715226174!3d7.848281200886689!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x17b30903777ac633%3A0xa08a798dca44bc0b!2sAtot%20hotel%20%26%20Restaurant!5e0!3m2!1sen!2set!4v1769427552240!5m2!1sen!2set",
        link: "manufacture",
      },
      {
        name: "Restaurant",
        key: "restaurant",
        desc1:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Adipisci, dolorum.",
        desc2:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur earum sapiente ratione excepturi impedit animi autem ea vero nisi aliquid maiores cum repellendus sed tenetur nostrum assumenda alias. Earum nihil voluptatibus aliquid rerum illo temporibus commodi eaque neque voluptate recusandae nam, totam cupiditate repudiandae blanditiis rem?",
        desc3:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur",
        video:
          "https://www.youtube-nocookie.com/embed/bWzN8NuZWF8?controls=0&modestbranding=1",
        map: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d988.1115485527807!2d38.18581715226174!3d7.848281200886689!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x17b30903777ac633%3A0xa08a798dca44bc0b!2sAtot%20hotel%20%26%20Restaurant!5e0!3m2!1sen!2set!4v1769427552240!5m2!1sen!2set",
        link: "restaurant",
      },
      {
        name: "Manufacture",
        key: "manufacture",
        desc1:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Adipisci, dolorum.",
        desc2:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur earum sapiente ratione excepturi impedit animi autem ea vero nisi aliquid maiores cum repellendus sed tenetur nostrum assumenda alias. Earum nihil voluptatibus aliquid rerum illo temporibus commodi eaque neque voluptate recusandae nam, totam cupiditate repudiandae blanditiis rem?",
        desc3:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur",
        video:
          "https://www.youtube-nocookie.com/embed/bWzN8NuZWF8?controls=0&modestbranding=1",
        map: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d988.1115485527807!2d38.18581715226174!3d7.848281200886689!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x17b30903777ac633%3A0xa08a798dca44bc0b!2sAtot%20hotel%20%26%20Restaurant!5e0!3m2!1sen!2set!4v1769427552240!5m2!1sen!2set",
        link: "mall",
      },
    ],
  },
  ar: {
    hero: {
      title: "Our Companies",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Adipisci, dolorum.",
    },
    navs: [
      {
        name: "Mall",
        link: "mall",
      },
      {
        name: "Restaurant",
        link: "restaurant",
      },
      {
        name: "Manufacturing",
        link: "manufacture",
      },
    ],
    video: {
      title: "Inside Our Company",
      desc: "Discover how Aberos PLC operates across multiple industries.",
      link: "https://www.youtube.com/embed/bWzN8NuZWF8?controls=0&modestbranding=1",
    },
    comp: [
      {
        name: "Mall",
        key: "mall",
        desc1:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Adipisci, dolorum.",
        desc2:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur earum sapiente ratione excepturi impedit animi autem ea vero nisi aliquid maiores cum repellendus sed tenetur nostrum assumenda alias. Earum nihil voluptatibus aliquid rerum illo temporibus commodi eaque neque voluptate recusandae nam, totam cupiditate repudiandae blanditiis rem?",
        desc3:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur",
        video:
          "https://www.youtube-nocookie.com/embed/bWzN8NuZWF8?controls=0&modestbranding=1",
        map: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d988.1115485527807!2d38.18581715226174!3d7.848281200886689!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x17b30903777ac633%3A0xa08a798dca44bc0b!2sAtot%20hotel%20%26%20Restaurant!5e0!3m2!1sen!2set!4v1769427552240!5m2!1sen!2set",
        link: "manufacture",
      },
      {
        name: "Restaurant",
        key: "restaurant",
        desc1:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Adipisci, dolorum.",
        desc2:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur earum sapiente ratione excepturi impedit animi autem ea vero nisi aliquid maiores cum repellendus sed tenetur nostrum assumenda alias. Earum nihil voluptatibus aliquid rerum illo temporibus commodi eaque neque voluptate recusandae nam, totam cupiditate repudiandae blanditiis rem?",
        desc3:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur",
        video:
          "https://www.youtube-nocookie.com/embed/bWzN8NuZWF8?controls=0&modestbranding=1",
        map: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d988.1115485527807!2d38.18581715226174!3d7.848281200886689!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x17b30903777ac633%3A0xa08a798dca44bc0b!2sAtot%20hotel%20%26%20Restaurant!5e0!3m2!1sen!2set!4v1769427552240!5m2!1sen!2set",
        link: "restaurant",
      },
      {
        name: "Manufacture",
        key: "manufacture",
        desc1:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Adipisci, dolorum.",
        desc2:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur earum sapiente ratione excepturi impedit animi autem ea vero nisi aliquid maiores cum repellendus sed tenetur nostrum assumenda alias. Earum nihil voluptatibus aliquid rerum illo temporibus commodi eaque neque voluptate recusandae nam, totam cupiditate repudiandae blanditiis rem?",
        desc3:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur",
        video:
          "https://www.youtube-nocookie.com/embed/bWzN8NuZWF8?controls=0&modestbranding=1",
        map: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d988.1115485527807!2d38.18581715226174!3d7.848281200886689!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x17b30903777ac633%3A0xa08a798dca44bc0b!2sAtot%20hotel%20%26%20Restaurant!5e0!3m2!1sen!2set!4v1769427552240!5m2!1sen!2set",
        link: "mall",
      },
    ],
  },
  am: {
    hero: {
      title: "Our Companies",
      desc: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Adipisci, dolorum.",
    },
    navs: [
      {
        name: "Mall",
        link: "mall",
      },
      {
        name: "Restaurant",
        link: "restaurant",
      },
      {
        name: "Manufacturing",
        link: "manufacture",
      },
    ],
    video: {
      title: "Inside Our Company",
      desc: "Discover how Aberos PLC operates across multiple industries.",
      link: "https://www.youtube.com/embed/bWzN8NuZWF8?controls=0&modestbranding=1",
    },
    comp: [
      {
        name: "Mall",
        key: "mall",
        desc1:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Adipisci, dolorum.",
        desc2:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur earum sapiente ratione excepturi impedit animi autem ea vero nisi aliquid maiores cum repellendus sed tenetur nostrum assumenda alias. Earum nihil voluptatibus aliquid rerum illo temporibus commodi eaque neque voluptate recusandae nam, totam cupiditate repudiandae blanditiis rem?",
        desc3:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur",
        video:
          "https://www.youtube-nocookie.com/embed/bWzN8NuZWF8?controls=0&modestbranding=1",
        map: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d988.1115485527807!2d38.18581715226174!3d7.848281200886689!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x17b30903777ac633%3A0xa08a798dca44bc0b!2sAtot%20hotel%20%26%20Restaurant!5e0!3m2!1sen!2set!4v1769427552240!5m2!1sen!2set",
        link: "manufacture",
      },
      {
        name: "Restaurant",
        key: "restaurant",
        desc1:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Adipisci, dolorum.",
        desc2:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur earum sapiente ratione excepturi impedit animi autem ea vero nisi aliquid maiores cum repellendus sed tenetur nostrum assumenda alias. Earum nihil voluptatibus aliquid rerum illo temporibus commodi eaque neque voluptate recusandae nam, totam cupiditate repudiandae blanditiis rem?",
        desc3:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur",
        video:
          "https://www.youtube-nocookie.com/embed/bWzN8NuZWF8?controls=0&modestbranding=1",
        map: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d988.1115485527807!2d38.18581715226174!3d7.848281200886689!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x17b30903777ac633%3A0xa08a798dca44bc0b!2sAtot%20hotel%20%26%20Restaurant!5e0!3m2!1sen!2set!4v1769427552240!5m2!1sen!2set",
        link: "restaurant",
      },
      {
        name: "Manufacture",
        key: "manufacture",
        desc1:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Adipisci, dolorum.",
        desc2:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur earum sapiente ratione excepturi impedit animi autem ea vero nisi aliquid maiores cum repellendus sed tenetur nostrum assumenda alias. Earum nihil voluptatibus aliquid rerum illo temporibus commodi eaque neque voluptate recusandae nam, totam cupiditate repudiandae blanditiis rem?",
        desc3:
          "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quo, quisquam incidunt exercitationem pariatur",
        video:
          "https://www.youtube-nocookie.com/embed/bWzN8NuZWF8?controls=0&modestbranding=1",
        map: "https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d988.1115485527807!2d38.18581715226174!3d7.848281200886689!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x17b30903777ac633%3A0xa08a798dca44bc0b!2sAtot%20hotel%20%26%20Restaurant!5e0!3m2!1sen!2set!4v1769427552240!5m2!1sen!2set",
        link: "mall",
      },
    ],
  },
};
